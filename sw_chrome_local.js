...
chrome.action.onClicked.addListener(async (tab) => {
  ...
  if(tab.url.includes(".public.lu") || tab.url.includes("gouvernement.lu") || tab.url.includes(".etat.lu") || tab.url.includes("sig-gr.eu") || tab.url.includes(".mae.lu") || tab.url.includes("lu-alert.lu")) {
	...
	const p5 = chrome.scripting.executeScript({
		files: [pluginUrl+'/script_local.js'],
		target: { tabId: tab.id }
	});
